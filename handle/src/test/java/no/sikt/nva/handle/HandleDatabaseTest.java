...
class HandleDatabaseTest {
  ...
  @Test
  void shouldReturnExistingHandleWhenCreatingHandleWithCustomPrefixAndHandleExists()
      throws SQLException {
  ...
  void shouldThrowRuntimeExceptionWhenDatabaseFailsToCreateHandleId() throws SQLException {
    ...
    assertThrows(
        RuntimeException.class, () -> handleDatabase.createHandle(CUSTOM_PREFIX, uri, connection));
  }
  ...
  private void mockHandleDatabaseForExistingHandle() throws SQLException {
    var checkUrlStatement =
        createPreparedStatementCheckUrl(
            true, HandleDatabaseTest.CUSTOM_PREFIX, HandleDatabaseTest.EXISTING_HANDLE_ID);
    when(connection.prepareStatement(CHECK_URL_SQL)).thenReturn(checkUrlStatement);
  }
  ...
  private PreparedStatement createPreparedStatementCheckUrl(
      boolean uriAlreadyExists, String prefix, int handleId) throws SQLException {
