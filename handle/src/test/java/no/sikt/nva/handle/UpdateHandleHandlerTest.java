...
class UpdateHandleHandlerTest {
  ...
  @Test
  void updateHandleRequestReturnsOkForValidUri() throws IOException, SQLException {
    var uri = randomUri();
  ...
  @Test
  void updateHandleRequestReturnsErrorIfNotFound() throws IOException, SQLException {
    var uri = randomUri();
    var inputStream = getUpdateHandleRequest(uri);
  ...
  private InputStream getUpdateHandleRequest(URI uri) throws JsonProcessingException {
    ...
    return new HandlerRequestBuilder<HandleRequest>(JsonUtils.dtoObjectMapper)
        .withBody(request)
        .withPathParameters(
            Map.of("prefix", HANDLE_PREFIX, "suffix", String.valueOf(UPDATE_HANDLE_ID)))
        .build();
  }

  private URI getFullHandleFromHandleId(int handleId) {
    return UriWrapper.fromHost(environment.readEnv(ENV_HANDLE_BASE_URI))
        .addChild(environment.readEnv(ENV_HANDLE_PREFIX), Integer.toString(handleId))
        .getUri();
  }
}
