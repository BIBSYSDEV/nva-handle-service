...
class CreateHandleHandlerTest {
  ...
  @Test
  void createHandleRequestReturnsCreatedHandleForValidUri() throws IOException, SQLException {
    var uri = randomUri();
    ...
    assertThat(
        response.getBodyObject(HandleResponse.class).handle(),
  ...
  @Test
  void createHandleForArbitraryPrefixesShouldFailOnDuplicate() throws IOException, SQLException {
    var uri = randomUri();
    var prefix = "some-prefix";
    var suffix = "some-suffix";
    var inputStream = getInputStream(new HandleRequest(uri, prefix, suffix));
  ...
  @Test
  void createHandleRequestReturnsExistingHandleForValidUriThatAlreadyHasHandle()
      throws IOException, SQLException {
    var uri = randomUri();
    var inputStream = createCreateHandleRequest(uri);
  ...
  @Test
  void createHandleRequestReturnsBadGatewayAndLogsErrorWhenNotAbleToCreateHandleId()
      throws IOException, SQLException {
    var uri = randomUri();
    var inputStream = createCreateHandleRequest(uri);
    ...
    var response = GatewayResponse.fromOutputStream(outputStream, Problem.class);
    assertThat(
        logRecorder.asString(), containsString(String.format(ERROR_CREATING_HANDLE_FOR_URI, uri)));
    assertThat(response.getStatusCode(), is(equalTo(HttpURLConnection.HTTP_BAD_GATEWAY)));
  }

  @Test
  void
      createHandleRequestReturnsBadGatewayAndLogsErrorAndRollbackDatabaseTransactionWhenNotAbleToCreateHandle()
          throws IOException, SQLException {
    var uri = randomUri();
    var inputStream = createCreateHandleRequest(uri);
  ...
  void createHandleRequestThrowsHandleExceptionAndLogsErrorWhenNotAbleToConnectToHandleDatabase() {
    ...
    var connectionSupplier = (Supplier<Connection>) mock(Supplier.class);
    var uri = randomUri();
    ...
    assertThrows(
        CreateHandleException.class, () -> failingHandler.processInput(request, null, null));
  ...
  private URI getHandleUri(String prefix, String suffix) {
    return UriWrapper.fromHost(environment.readEnv(ENV_HANDLE_BASE_URI))
        .addChild(prefix, suffix)
        .getUri();
  }

  private void mockHandleDatabaseCreateHandle(
      boolean uriAlreadyExists, boolean successfulCreateHandleId, boolean successfulCreateHandle)
      throws SQLException {
    ...
    PreparedStatement preparedStatementCheckHandle =
        createPreparedStatementCheckUrl(uriAlreadyExists);
    when(connection.prepareStatement(CHECK_HANDLE_SQL)).thenReturn(preparedStatementCheckHandle);

    PreparedStatement preparedStatementCreateId =
        createPreparedStatementCreateId(successfulCreateHandleId);
    when(connection.prepareStatement(CREATE_ID_SQL)).thenReturn(preparedStatementCreateId);

    PreparedStatement preparedStatementSetHandle =
        createPreparedStatementSetHandle(successfulCreateHandle);
    when(connection.prepareStatement(SET_HANDLE_AND_URI_BY_ID_SQL))
        .thenReturn(preparedStatementSetHandle);
  }
