...
class ClinicalTrialTest {
  ...
  private static final String CLINICAL_TRIAL_JSON =
      stringFromResources(Path.of("clinical-trial.json"));

  @Test
  void shouldDeserializeFromJson() throws Exception {
    var clinicalTrial =
        JsonUtils.dtoObjectMapper.readValue(CLINICAL_TRIAL_JSON, ClinicalTrial.class);

    assertThat(
        clinicalTrial.id(),
        is(URI.create("https://st-api.legemiddelverket.no/ctis/clinical-trial/2022-500027-76-00")));
  ...
  void shouldDeserializeTrialSites() throws Exception {
    ...
    assertThat(
        trialSite.investigator().nvaPersonId(),
        is(URI.create("https://api.nva.unit.no/cristin/person/23297")));
  }
  ...
  void shouldDeserializePublicContactPoint() throws Exception {
    ...
    assertThat(
        clinicalTrial.publicContactPoint().organizationName(), is("Akershus University Hospital"));
